...
export default function PodcastComposer() {
  ...
  // Wrapped in useMemo to prevent re-rendering glitches when switching browser tabs
  ...
  const handlePublish = async () => {
    // Strict validation enforcing all required links
    if (!formData.title || !formData.embed_url || !formData.spotify_url) {
      alert("Please enter a title, YouTube URL, and Spotify URL before publishing.");
      return;
    }
    ...
    try {
      ...
      alert("Failed to publish episode. Please check the console for details.");
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
        <div>
          <h1 className="text-3xl font-heading font-black text-primary mb-1">Create Episode</h1>
          <p className="text-sm text-gray-500 font-medium">Publish a new podcast conversation.</p>
        </div>
          ...
          >
            {isSubmitting 
              ...
              : (publishMode === 'now' 
                  ...
                  : <><ClockIcon className="w-4 h-4"/> Schedule Episode</>
                )
            }
          </button>
        </div>
