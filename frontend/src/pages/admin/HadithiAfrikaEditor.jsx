...
export default function HadithiAfrikaEditor() {
  ...
  useEffect(() => {
    const fetchFilm = async () => {
      try {
        const response = await contentAPI.getFilm(id);
        ...
        const existingDate = film.published_date ? new Date(film.published_date) : new Date();
        ...
        if (film.poster_image) {
          setMediaPreview(film.poster_image);
        }
  ...
  const handleUpdate = async () => {
    ...
    submitData.append('is_published', finalIsPublished ? 'true' : 'false');
    ...
    try {
      await contentAPI.updateFilm(id, submitData);
      navigate('/admin/hadithi-afrika');
    } catch (error) {
      console.error("Error updating film:", error);
      alert("Failed to update film. Check console.");
  ...
  return (
    <div className="max-w-4xl mx-auto pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-heading font-black text-primary mb-1">Edit Film</h1>
          <p className="text-sm text-gray-500 font-medium">Update the details or status for this production.</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-12">
        <div className="p-8 space-y-8">
            ...
            <div className="space-y-2">
              ...
              <div className={`grid transition-all duration-300 ease-in-out ${postStatus === 'scheduled' ? 'grid-rows-[1fr] opacity-100 mt-4' : 'grid-rows-[0fr] opacity-0 mt-0'}`}>
                <div className="overflow-hidden">
                  ...
                  <p className="text-[10px] text-gray-400 mt-2 font-medium">Film will be hidden and auto-published at this time.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold uppercase tracking-wider text-gray-500">Poster Image</label>
            <input 
              type="file" 
              accept="image/*" 
              className="hidden" 
            ...
            >
              ...
              <div className="relative z-10 bg-white/80 px-4 py-2 rounded-lg backdrop-blur-sm">
                ...
                <p className="text-xs text-gray-500">Leave as is to keep current poster</p>
              </div>
            </div>
          </div>
